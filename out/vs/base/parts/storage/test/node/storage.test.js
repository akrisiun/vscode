...
define(["require", "exports", "vs/base/parts/storage/node/storage", "vs/base/parts/storage/common/storage", "vs/base/common/uuid", "vs/base/common/path", "os", "assert", "vs/base/node/pfs", "vs/base/common/async", "vs/base/common/event", "vs/base/common/platform"], function (require, exports, storage_1, storage_2, uuid_1, path_1, os_1, assert_1, pfs_1, async_1, event_1, platform_1) {
    ...
    suite('Storage Library', () => {
        ...
        test('external changes', async () => {
            ...
            assert_1.ok(changes.has('foo'));
            assert_1.equal(storage.get('foo', undefined), undefined);
            changes.clear();
            // Nothing happens if changing to same value
            database.fireDidChangeItemsExternal({ deleted });
            assert_1.equal(changes.size, 0);
        ...
        test('corrupt DB recovers', async () => {
            ...
            await pfs_1.writeFile(storageFile, 'This is a broken DB');
            await storage.set('foo', 'bar');
            assert_1.equal(storage.get('bar'), 'foo');
            assert_1.equal(storage.get('foo'), 'bar');
            await storage.close();
    ...
    suite('SQLite Storage Library', () => {
        ...
        test('basics', async () => {
            const storageDir = uniqueStorageDir();
            await pfs_1.mkdirp(storageDir);
            await testDBBasics(path_1.join(storageDir, 'storage.db'));
            await pfs_1.rimraf(storageDir, pfs_1.RimRafMode.MOVE);
        });
        ...
        test('basics (corrupt DB falls back to empty DB)', async () => {
            ...
            await pfs_1.writeFile(corruptDBPath, 'This is a broken DB');
            let expectedError;
        ...
        test('basics (corrupt DB restores from previous backup)', async () => {
            ...
            await storage.updateItems({ insert: items });
            await storage.close();
            await pfs_1.writeFile(storagePath, 'This is now a broken DB');
        ...
        test('basics (corrupt DB falls back to empty DB if backup is corrupt)', async () => {
            ...
            await storage.close();
            await pfs_1.writeFile(storagePath, 'This is now a broken DB');
            await pfs_1.writeFile(`${storagePath}.backup`, 'This is now also a broken DB');
            storage = new storage_1.SQLiteStorageDatabase(storagePath);
            const storedItems = await storage.getItems();
            assert_1.equal(storedItems.size, 0);
        ...
        test('basics (DB that becomes corrupt during runtime stores all state from cache on close)', async () => {
            ...
            assert_1.equal(await pfs_1.exists(backupPath), true);
            storage = new storage_1.SQLiteStorageDatabase(storagePath);
            ...
            await pfs_1.unlink(backupPath); // also test that the recovery DB is backed up properly
            let recoveryCalled = false;
            await storage.close(() => {
                recoveryCalled = true;
                return items;
        ...
        test('real world example', async function () {
            ...
            assert_1.equal(await storage.checkIntegrity(true), 'ok');
            assert_1.equal(await storage.checkIntegrity(false), 'ok');
            storedItems = await storage.getItems();
