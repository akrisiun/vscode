        ...
        let CodeApplication = class CodeApplication extends lifecycle_1.Disposable {
            ...
            async resolveMachineId() {
                ...
                if (platform_1.isMacintosh && machineId === '6c9d2bc8f91b89624add29c0abeae7fb42bf539fa1cdb2e3e57cd668fa9bcead') {
                    trueMachineId = this.stateService.getItem(telemetry_1.trueMachineIdKey);
                    if (!trueMachineId) {
                        trueMachineId = await id_1.getMachineId();
                        this.stateService.setItem(telemetry_1.trueMachineIdKey, trueMachineId);
                    }
                }
                return { machineId, trueMachineId };
            }
            async createServices(machineId, trueMachineId, sharedProcess, sharedProcessReady) {
                ...
                if (!this.environmentService.isExtensionDevelopment && !this.environmentService.args['disable-telemetry'] && !!product_1.default.enableTelemetry) {
                    ...
                    const config = { appender, commonProperties, piiPaths, trueMachineId, sendErrorTelemetry: true };
            ...
            shouldBlockURI(uri) {
                if (uri.authority === network_1.Schemas.file && platform_1.isWindows) {
                    const res = electron_1.dialog.showMessageBoxSync({
                        ...
                        type: 'question',
                        buttons: [
                            labels_1.mnemonicButtonLabel(nls_1.localize({ key: 'open', comment: ['&& denotes a mnemonic'] }, "&&Yes")),
                            labels_1.mnemonicButtonLabel(nls_1.localize({ key: 'cancel', comment: ['&& denotes a mnemonic'] }, "&&No")),
                        ],
                        cancelId: 1,
                        message: nls_1.localize('confirmOpenMessage', "An external application wants to open '{0}' in {1}. Do you want to open this file or folder?", labels_1.getPathLabel(uri.fsPath), product_1.default.nameShort),
                        detail: nls_1.localize('confirmOpenDetail', "If you did not initiate this request, it may represent an attempted attack on your system. Unless you took an explicit action to initiate this request, you should press 'No'"),
                        noLink: true
                    });
                    if (res === 1) {
                        return true;
