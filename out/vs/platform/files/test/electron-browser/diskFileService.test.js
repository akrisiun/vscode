 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "os", "vs/platform/files/common/fileService", "vs/base/common/network", "vs/platform/files/electron-browser/diskFileSystemProvider", "vs/base/test/node/testUtils", "vs/base/common/uuid", "vs/base/common/path", "vs/base/common/amd", "vs/base/node/pfs", "vs/base/common/uri", "fs", "vs/platform/files/common/files", "vs/platform/log/common/log", "vs/base/common/platform", "vs/base/common/lifecycle", "vs/base/common/resources", "vs/base/common/buffer", "vs/base/common/arrays"], function (require, exports, assert, os_1, fileService_1, network_1, diskFileSystemProvider_1, testUtils_1, uuid_1, path_1, amd_1, pfs_1, uri_1, fs_1, files_1, log_1, platform_1, lifecycle_1, resources_1, buffer_1, arrays_1) {
    "use strict";
    ...
    suite('Disk File Service', function () {
        ...
        test('resolve - file', async () => {
            const resource = uri_1.URI.file(amd_1.getPathFromAmdModule(require, './fixtures/resolver/index.html'));
        ...
        test('resolve - directory', async () => {
            ...
            const resource = uri_1.URI.file(amd_1.getPathFromAmdModule(require, './fixtures/resolver'));
        ...
        (platform_1.isWindows /* not reliable on windows */ ? test.skip : test)('resolve - folder symbolic link', async () => {
        ...
        (platform_1.isWindows /* not reliable on windows */ ? test.skip : test)('resolve - file symbolic link', async () => {
        ...
        (platform_1.isWindows /* not reliable on windows */ ? test.skip : test)('resolve - symbolic link pointing to non-existing file does not break', async () => {
            var _a;
            await pfs_1.symlink(path_1.join(testDir, 'foo'), path_1.join(testDir, 'bar'));
            const resolved = await service.resolve(uri_1.URI.file(testDir));
            assert.equal(resolved.isDirectory, true);
            assert.equal(resolved.children.length, 9);
            const resolvedLink = (_a = resolved.children) === null || _a === void 0 ? void 0 : _a.filter(child => child.name === 'bar' && child.isSymbolicLink)[0];
        ...
        (platform_1.isWindows /* not reliable on windows */ ? test.skip : test)('deleteFile - symbolic link (exists)', async () => {
            const target = uri_1.URI.file(path_1.join(testDir, 'lorem.txt'));
            const link = uri_1.URI.file(path_1.join(testDir, 'lorem.txt-linked'));
            await pfs_1.symlink(target.fsPath, link.fsPath);
            ...
            assert.equal(event.operation, 1 /* DELETE */);
            assert.equal(fs_1.existsSync(target.fsPath), true); // target the link pointed to is never deleted
        });
        (platform_1.isWindows /* not reliable on windows */ ? test.skip : test)('deleteFile - symbolic link (pointing to non-existing file)', async () => {
            const target = uri_1.URI.file(path_1.join(testDir, 'foo'));
            const link = uri_1.URI.file(path_1.join(testDir, 'bar'));
            await pfs_1.symlink(target.fsPath, link.fsPath);
            let event;
            disposables.add(service.onDidRunOperation(e => event = e));
            await service.del(link);
            assert.equal(fs_1.existsSync(link.fsPath), false);
            assert.ok(event);
            assert.equal(event.resource.fsPath, link.fsPath);
            assert.equal(event.operation, 1 /* DELETE */);
        });
        ...
        test('readFileStream - small file - default', () => {
            return testReadFileStream(uri_1.URI.file(path_1.join(testDir, 'small.txt')));
        });
        ...
        async function testReadFileStream(resource) {
            ...
            assert.equal((await buffer_1.streamToBuffer(content.value)).toString(), fs_1.readFileSync(resource.fsPath));
        }
        ...
        (platform_1.isWindows /* error code does not seem to be supported on windows */ ? test.skip : test)('readFile - FILE_NOT_DIRECTORY', async () => {
            const resource = uri_1.URI.file(path_1.join(testDir, 'lorem.txt', 'file.txt'));
            let error = undefined;
            try {
                await service.readFile(resource);
            }
            ...
            assert.ok(error);
            assert.equal(error.fileOperationResult, 10 /* FILE_NOT_DIRECTORY */);
        });
        ...
        async function doTestFileExceedsMemoryLimit() {
            ...
            assert.ok(error);
            assert.equal(error.fileOperationResult, 9 /* FILE_EXCEEDS_MEMORY_LIMIT */);
        }
        ...
        test('createFile (readable)', async () => {
            return assertCreateFile(contents => buffer_1.bufferToReadable(buffer_1.VSBuffer.fromString(contents)));
        });
        test('createFile (stream)', async () => {
            return assertCreateFile(contents => buffer_1.bufferToStream(buffer_1.VSBuffer.fromString(contents)));
        });
        ...
        async function testWriteFileStream() {
            ...
            const fileStat = await service.writeFile(target, buffer_1.streamToBufferReadableStream(fs_1.createReadStream(source.fsPath)));
        ...
        test('writeFile - no error when writing to same non-existing folder multiple times different new files', async () => {
            const newFolder = uri_1.URI.file(path_1.join(testDir, 'some', 'new', 'folder'));
            const file1 = resources_1.joinPath(newFolder, 'file-1');
            const file2 = resources_1.joinPath(newFolder, 'file-2');
            const file3 = resources_1.joinPath(newFolder, 'file-3');
        ...
        test('writeFile - error when writing to folder that is a file', async () => {
            const existingFile = uri_1.URI.file(path_1.join(testDir, 'my-file'));
            await service.createFile(existingFile);
            const newFile = resources_1.joinPath(existingFile, 'file-1');
            let error;
            const newContent = 'Updates to the small file';
            try {
