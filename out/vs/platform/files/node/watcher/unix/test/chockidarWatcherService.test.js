...
define(["require", "exports", "assert", "os", "vs/base/common/path", "vs/base/node/pfs", "../chokidarWatcherService", "vs/base/common/platform", "vs/base/common/async"], function (require, exports, assert, os, path, pfs, chokidarWatcherService_1, platform, async_1) {
    ...
    async function assertFileEvents(actuals, expected) {
        let repeats = 40;
    ...
    suite.skip('Chokidar watching', () => {
        ...
        test('simple file operations, single root, no ignore', async () => {
            ...
            await assertFileEvents(result, [{ path: testFilePath, type: 1 /* ADDED */ }]);
            // modify a file
            await pfs.writeFile(testFilePath, 'Hello');
            await assertFileEvents(result, [{ path: testFilePath, type: 0 /* UPDATED */ }]);
            // create a folder
            ...
            await assertFileEvents(result, [{ path: copiedFilePath, type: 1 /* ADDED */ }, { path: testFolderPath, type: 1 /* ADDED */ }]);
            // delete a file
            ...
            await assertFileEvents(result, [{ path: copiedFilePath, type: 2 /* DELETED */ }, { path: testFilePath, type: 2 /* DELETED */ }, { path: renamedFilePath, type: 1 /* ADDED */ }]);
            // delete a folder
            await pfs.rimraf(testFolderPath, pfs.RimRafMode.MOVE);
            await assertFileEvents(result, [{ path: testFolderPath, type: 2 /* DELETED */ }, { path: renamedFilePath, type: 2 /* DELETED */ }]);
        });
        test('simple file operations, ignore', async () => {
            ...
            await assertFileEvents(result, [{ path: file4, type: 1 /* ADDED */ }, { path: folder4, type: 1 /* ADDED */ }, { path: folder5, type: 1 /* ADDED */ }]);
            // move some files
            ...
            await assertFileEvents(result, [{ path: movedFile2, type: 1 /* ADDED */ }, { path: movedFile3, type: 1 /* ADDED */ }]);
            // delete all files
            ...
            await assertFileEvents(result, [{ path: movedFile2, type: 2 /* DELETED */ }, { path: movedFile3, type: 2 /* DELETED */ }, { path: file4, type: 2 /* DELETED */ }, { path: folder4, type: 2 /* DELETED */ }, { path: folder5, type: 2 /* DELETED */ }]);
        });
        test('simple file operations, multiple roots', async () => {
            ...
            await assertFileEvents(result, [{ path: folderPath1, type: 1 /* ADDED */ }, { path: filePath1, type: 1 /* ADDED */ }, { path: folderPath2, type: 1 /* ADDED */ }]);
            // change roots
            ...
            await assertFileEvents(result, [{ path: folderPath1, type: 2 /* DELETED */ }, { path: filePath2, type: 2 /* DELETED */ }]);
        });
        test('simple file operations, nested roots', async () => {
            ...
            await pfs.writeFile(filePath3, '');
            await assertFileEvents(result, [{ path: filePath1, type: 1 /* ADDED */ }, { path: filePath2, type: 1 /* ADDED */ }]);
            let renamedFilePath2 = path.join(folderPath1, 'file2.xml');
            // move a file
            await pfs.rename(filePath2, renamedFilePath2);
            await assertFileEvents(result, [{ path: filePath2, type: 2 /* DELETED */ }]);
            // delete all
            await pfs.rimraf(folderPath1, pfs.RimRafMode.MOVE);
            await pfs.rimraf(filePath1, pfs.RimRafMode.MOVE);
            await assertFileEvents(result, [{ path: filePath1, type: 2 /* DELETED */ }]);
        });
    });
});
