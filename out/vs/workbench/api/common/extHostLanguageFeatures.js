...
define(["require", "exports", "vs/base/common/uri", "vs/base/common/objects", "vs/workbench/api/common/extHostTypeConverters", "vs/workbench/api/common/extHostTypes", "vs/base/common/async", "./extHost.protocol", "vs/base/common/strings", "vs/editor/common/core/range", "vs/base/common/arrays", "vs/base/common/types", "vs/editor/common/core/selection", "vs/base/common/lifecycle"], function (require, exports, uri_1, objects_1, typeConvert, extHostTypes_1, async_1, extHostProtocol, strings_1, range_1, arrays_1, types_1, selection_1, lifecycle_1) {
    ...
    class CallHierarchyAdapter {
        ...
        async provideCallsTo(uri, position, token) {
            ...
            return calls.map(call => [typeConvert.CallHierarchyItem.from(call.source), call.sourceRanges.map(typeConvert.Range.from)]);
        }
        async provideCallsFrom(uri, position, token) {
            const doc = this._documents.getDocument(uri);
            const pos = typeConvert.Position.to(position);
            ...
            return calls.map(call => [typeConvert.CallHierarchyItem.from(call.target), call.sourceRanges.map(typeConvert.Range.from)]);
        }
    }
    class AdapterData {
    ...
    class ExtHostLanguageFeatures {
        ...
        $provideCallHierarchyIncomingCalls(handle, resource, position, token) {
            return this._withAdapter(handle, CallHierarchyAdapter, adapter => adapter.provideCallsTo(uri_1.URI.revive(resource), position, token), undefined);
        }
        $provideCallHierarchyOutgoingCalls(handle, resource, position, token) {
            return this._withAdapter(handle, CallHierarchyAdapter, adapter => adapter.provideCallsFrom(uri_1.URI.revive(resource), position, token), undefined);
        }
        // --- configuration
