        ...
        $registerCallHierarchyProvider(handle, selector) {
            this._registrations.set(handle, callh.CallHierarchyProviderRegistry.register(selector, {
                provideOutgoingCalls: async (model, position, token) => {
                    ...
                    return outgoing.map(([item, sourceRanges]) => {
                        return {
                            target: MainThreadLanguageFeatures_1._reviveCallHierarchyItemDto(item),
                            sourceRanges
                        };
                    });
                },
                provideIncomingCalls: async (model, position, token) => {
                    ...
                    return incoming.map(([item, sourceRanges]) => {
                        return {
                            source: MainThreadLanguageFeatures_1._reviveCallHierarchyItemDto(item),
                            sourceRanges
                        };
                    });
                }
