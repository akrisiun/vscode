    ...
    let ExtensionManagementService = class ExtensionManagementService extends lifecycle_1.Disposable {
        ...
        async uninstallInServer(extension, server, force) {
            if (server === this.extensionManagementServerService.localExtensionManagementServer) {
                ...
                const dependentNonUIExtensions = installedExtensions.filter(i => !extensionsUtil_1.isUIExtension(i.manifest, this.productService, this.configurationService)
        ...
        async install(vsix) {
            if (this.extensionManagementServerService.localExtensionManagementServer && this.extensionManagementServerService.remoteExtensionManagementServer) {
                ...
                if (extensionsUtil_1.isUIExtension(manifest, this.productService, this.configurationService)) {
