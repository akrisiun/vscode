    ...
    let DataSource = class DataSource {
        ...
        async _getOutgoingCalls(model, position, bucket, parent) {
            ...
            for (const call of outgoingCalls) {
                bucket.push(new Call(call.target, call.sourceRanges.map(range => ({ range, uri: model.uri })), parent));
            }
        }
        async _getIncomingCalls(model, position, bucket, parent) {
            ...
            for (const call of incomingCalls) {
                bucket.push(new Call(call.source, call.sourceRanges.map(range => ({ range, uri: call.source.uri })), parent));
            }
        }
    };
