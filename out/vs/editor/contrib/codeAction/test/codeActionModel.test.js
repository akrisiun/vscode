...
define(["require", "exports", "assert", "vs/base/common/lifecycle", "vs/base/common/types", "vs/base/common/uri", "vs/editor/common/modes", "vs/editor/contrib/codeAction/codeActionModel", "vs/editor/test/browser/testCodeEditor", "vs/platform/keybinding/test/common/mockKeybindingService", "vs/platform/markers/common/markerService", "vs/editor/test/common/editorTestUtils"], function (require, exports, assert, lifecycle_1, types_1, uri_1, modes, codeActionModel_1, testCodeEditor_1, mockKeybindingService_1, markerService_1, editorTestUtils_1) {
    ...
    suite('CodeActionModel', () => {
        ...
        setup(() => {
            ...
            model = editorTestUtils_1.createTextModel('foobar  foo bar\nfarboo far boo', undefined, languageIdentifier, uri);
        ...
        test('Orcale -> marker added', done => {
            ...
            disposables.add(model.onDidChangeState((e) => {
                types_1.assertType(e.type === 1 /* Triggered */);
                assert.strictEqual(e.trigger.type, 1 /* Auto */);
                assert.ok(e.actions);
        ...
        test('Orcale -> position changed', () => {
            ...
            return new Promise((resolve, reject) => {
                ...
                disposables.add(model.onDidChangeState((e) => {
                    types_1.assertType(e.type === 1 /* Triggered */);
                    assert.equal(e.trigger.type, 1 /* Auto */);
                    assert.ok(e.actions);
                    e.actions.then(fixes => {
                        model.dispose();
                        assert.equal(fixes.validActions.length, 1);
